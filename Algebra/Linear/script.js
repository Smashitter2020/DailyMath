...
function getDailyOperator() {
  ...
  num2 = Math.floor(rng(seed) * 30) + 2;
  num1 = Math.floor(rng(seed) * 30) + 2;
  num3 = Math.floor(rng(seed) * 30) + 2;
  return operators[keys[index]];
}

function checkAnswer() {
  const userAnswer = parseFloat(document.getElementById("answer").value);
  const correctAnswer = num3;
...
num4 = Math.round(eval(num1 + dailyOp.operator + (num2 * num3)) * 100) / 100;

document.getElementById("question").innerHTML = `<math xmlns="http://www.w3.org/1998/Math/MathML"><mn>${num1}</mn><mo>${dailyOp.symbol}</mo><mn>${num2}</mn><mi>x</mi><mo>=</mo><mn>${num4}</mn></math>`;
