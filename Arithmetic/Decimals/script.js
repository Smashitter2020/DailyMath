...
function getDailyOperator() {
  ...
  // Avoid division by zero in decimals
  num1 = (Math.floor(rng(seed) * 10000) / 100) + 1;
  num2 = (Math.floor(rng(seed) * num1) / 100) + 1;
...
document.getElementById("question").innerHTML = `<math xmlns="http://www.w3.org/1998/Math/MathML"><mn>${num1}</mn><mo>${dailyOp.symbol}</mo><mn>${num2}</mn></math>`;
